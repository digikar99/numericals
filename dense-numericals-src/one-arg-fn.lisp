...
(macrolet ((def (fn-retriever bits)
             `(defpolymorph dn:abs ((x (array (signed-byte ,bits)))
                                    &key ((out (array (signed-byte ,bits))) (zeros-like x)))
                ...
                (with-thresholded-multithreading (array-total-size out)
                    (x out)
                  (ptr-iterate-but-inner n ((ptr-x   ,(/ bits 8) ix   x)
                                            (ptr-out ,(/ bits 8) iout out))
                    (,(funcall fn-retriever 'dn:abs) n ptr-x ix ptr-out iout)))
                out)))
