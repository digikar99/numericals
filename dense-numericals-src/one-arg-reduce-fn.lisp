...
(defpolymorph (one-arg-reduce-fn :inline t) ((name symbol) (initial-value-name symbol)
    ...
    t
  (one-arg-reduce-fn name initial-value-name (print (asarray x)) :axes axes :out out))
...
(5am:def-test dn:max ()
  (flet ((float-close-p (x y)
    ...
    (loop :for *array-element-type* :in `(single-float
                                          ...
                                          fixnum)
          :do
             (5am:is (= 3 (dn:max (asarray '(1 2 3)))))
             (5am:is (= 9 (dn:max (asarray '((1 2 3)
                                             (4 5 6)
                                             (7 8 9))))))
             ;; (5am:is (array= (asarray '(5 7 9))
             ;;                 (dn:sum (asarray '((1 2 3)
             ...
             (let ((array (dn:rand 100)))
               (5am:is (float-close-p (dn:max array)
  ...
  (flet ((float-close-p (x y)
    ...
    (loop :for *array-element-type* :in `(single-float
          ...
          :do
             ...
             (let ((array (dn:rand 100)))
               (5am:is (float-close-p (dn:min array)
