...
(define-polymorphic-function nu:concat (array-likes &key out (axis 0)) :overwrite t)
...
(defpolymorph (nu:concat :inline t) (array-likes
...
(defpolymorph (nu:concat :inline t) (array-likes &key ((out null)) ((axis integer)))
    t
  (declare (ignore out))
  (let ((array-likes (loop :for array :in array-likes
                           :collect (if (typep array 'nu:array)
                                        array
                                        (nu:asarray array)))))
    (nu:concat array-likes
               :out (nu:zeros (out-shape-for-concat axis array-likes)
                              :type (array-element-type (first array-likes)))
               :axis axis)))

(5am:def-test nu:concat ()
  (dolist (nu:*array-element-type* '(single-float
                                     double-float
                                     (signed-byte 64)
                                     (signed-byte 32)
                                     (signed-byte 16)
    ...
    (5am:is (nu:array= (nu:asarray '(1 2 3 4 5 6))
                       (nu:concat (list '(1 2 3) '(4 5 6))
                                  :axis 0)))
    (5am:is (nu:array= (nu:asarray '((1 2 3)
                                     (4 5 6)))
                       (nu:concat (list '((1 2 3))
                                        '((4 5 6)))
                                  :axis 0)))
    (5am:is (nu:array= (nu:asarray '((1 2 3 4 5 6)))
                       (nu:concat (list '((1 2 3))
