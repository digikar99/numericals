...
(defun out-shape-compatible-for-arg-maximum-p (out in axis keep-dims)
...
(defpolymorph nu:arg-maximum ((array (nu:simple-array <type>))
  ...
  (policy-cond:with-expectations (= safety 0)
      ((assertion (out-shape-compatible-for-arg-maximum-p out array axis keep-dims)
                  (array out)
...
(declaim (ftype (function (nu:array (integer 0 #.array-rank-limit) boolean) list)
                out-shape-for-arg-maximum))
(defun out-shape-for-arg-maximum (in-array axis keep-dims)
  (declare (optimize speed)
...
(defpolymorph (nu:arg-maximum :inline t
    ...
    t
  (declare (ignore out))
  (pflet* ((out (nu:zeros (out-shape-for-arg-maximum array axis keep-dims)
                          :type '(signed-byte 64))))
    (declare (type (simple-array (signed-byte 64)) out))
