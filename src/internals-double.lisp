...
(declaim (inline f4-ref))
(defun f4-ref (vec i)
  (declare (optimize (speed 3))
           (type (simple-array double-float) vec)
           (type fixnum i))
  (sb-vm::f4-ref vec i))
(declaim (inline (setf f4-ref)))
(defun (setf f4-ref) (new-value vec i)
  (declare (optimize (speed 3))
           (type (simple-array double-float) vec)
           (type fixnum i))
  (sb-vm::f4-set vec i new-value))
       ...
       (defun ,sbcl-numericals-symbol (array-a array-b result-array)
         ...
         (let ((vec-a (array-storage-vector array-a))
           ...
           (loop for i below (ceiling (length vec-a) 4)
              do (setf (f4-ref vec-r i)
                       (,internals-symbol (f4-ref vec-a i)
                                          (f4-ref vec-b i)))
