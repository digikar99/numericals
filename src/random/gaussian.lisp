...
(define-polymorphic-function rand:gaussian (&key (loc 0) (scale 1)
...
(defpolymorph rand:gaussian (&key
  ...
  (policy-cond:with-expectations (= safety 0)
    ...
    (imlet ((mean (coerce (if meanp mean loc) 'double-float))
            (std  (coerce (if stdp std scale) 'double-float)))
      ;; If there were a way to stack allocate a foreign value (portably),
      ;; then we could have used the CEIGEN-LITE function.
      (coerce (+ mean
                 (cffi:foreign-funcall "gsl_ran_gaussian"
                                       :pointer +gsl-rng+
                                       :double std
                                       :double))
              type))))
  ...
  (policy-cond:with-expectations (= safety 0)
      ((assertion (not (and shapep sizep))
       ...
       (assertion (not (and stdp scalep))
                  ()
                  "Only one of STD or SCALE should be supplied"))
    (let* ((mean  (coerce (if meanp mean loc) type))
           (std   (coerce (if stdp std scale) type))
           (shape (if shapep shape size))
           (out   (nu:empty shape :type type))
           (c-name (c-name type 'rand:gaussian))
